...
var btn_enviar_usuario_sistema = $(".guardar-form");
btn_enviar_usuario_sistema.on('click', function() {
    ...
    validarInput($('input#porcentaje_comision'), regExprNum) == false ? inputs.push('Porcentaje comisión') : ''
    validarSelect($('select#categorias_id')) == false ? inputs.push('Categorías') : ''
    validarSelect($('select#subcategorias_id')) == false ? inputs.push('Subcategorías') : ''
...
$( "input#porcentaje_comision" ).blur(function() {
    validarInput($(this), regExprNum);
});
$( "select#categorias_id" ).change(function() {
    validarSelect($(this));
});
$( "select#subcategorias_id" ).change(function() {
    validarSelect($(this));
});


function validarInput (campo,regExpr) {
    if($('form#form-data input#id').val() != '' && $(campo).attr('name') == 'password' && $(campo).val() == '') {
        return true;
...
function validarSelect (select) {
    if ($(select).val() == '0' || $(select).val() == '' || $(select).val() == null) {
        if ($(select).hasClass('select2')) {
            ...
            $(select).parent().children('button').addClass("selectpicker-error");
        }
        msgError = msgError + $(select).parent().parent().children('label').text() + '\n';
        return false;
    } else {
        if ($(select).hasClass('select2')) {
            $(select).parent().children('div.select2').children('ul.select2-choices').removeClass("select-error");
        } else {
            $(select).parent().children('button').removeClass("selectpicker-error");
        }
        return true;
    }
}
...
function validarArchivo(campo) {
    archivo = $(campo).val();
    extension = archivo.split('.').pop().toLowerCase();
    if($('form#form-data input#id').val() != '' && ($(campo).val() == '' || $(campo).val() == null)) {
        return true;
